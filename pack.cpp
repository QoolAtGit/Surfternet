...
struct send_data_t
{
 ...
 {
     unsigned short sign;//ipclient sign
     unsigned char number;//Packet number
     char unknow[8]; //look like be 0 forever
 };
...
void* heart(void *arg)
{
    ...
    while(1)
    {
        ...
        sprintf(buf,"Remaining %.3f RMB",money);
        cout<<buf<<"\n\n";
        sleep(40);
    }
}
...
int open_ip()
{
    ...
    {
        //create_message_dialog(GTK_MESSAGE_INFO,"Username or password empty.");
        cout<<"null"<<endl;
        return 1;
    }
    build_0x1f();
    ...
    if(result == 0)
    {
        //create_message_dialog(GTK_MESSAGE_INFO,"Open fail.");
        cout<<"fail"<<endl;
        return 0;
    }
    else
    {
        ...
        return 1;
        //create_message_dialog(GTK_MESSAGE_INFO,"Open success.");
    }
...
void readNumber()
{
    ...
    while(true)
    {
        ...
        {
            cout<<"Not accounts availabel!"<<endl;
            inUser.close();
            break;
        }
        if(c++<index)
...
int main()
{
    ...
    while(1)
    {
        int net = system("ping -c 2 114.114.114.114");
        cout<<net<<"\n\n";
        if(net)
        {
            if(isFirst==true)
...
void build_0x21()
{
    ...
    generate_md5_string(md5);//encrypt
    add_data(&md5,0x1e);
...
int generate_md5_string(char *str)  //encrypt by username and password
//存于str中
{
    char sz_str[64];
    char sz_md5[64];
